...
public class GameManager : MonoBehaviour
{
    ...
    public void GameOver()
    {
        ...
        // 최고 기록 경신
        int maxScore = PlayerPrefs.GetInt("MaxScore"); // 프리팹에 저장된 맥스 스코어를 가져옴
        if (player.score > maxScore) // 맥스스코어 보다 현재 스코어가 높으면 갱신
        {
            bestScoreText.gameObject.SetActive(true); // best! 라는 문구 활성화 
            PlayerPrefs.SetInt("MaxScore", player.score);
        }
    }

    public void Restart() // 다시시작 버튼 
    {
    ...
    {
        stage++;
        ShopZone.SetActive(false);  // 게임 시작시 상점 비활성화
