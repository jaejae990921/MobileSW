...
public class Player : MonoBehaviour
{
    ...
    void GetInput()
    {
        hAxis = joy.Horizontal; // Axis값을 정수로 반환하는 함수(오른쪽왼쪽 컨트롤)
        vAxis = joy.Vertical; //(위아래 컨트롤)
    ...
    void Move()
    {
        ...
        if (isShot || isSwing || isSwap || isReload || !isFireReady || isShop || isDead) //무기 교체시 움직임도 멈춤 ,장전중, 공격중에는 이동불가
