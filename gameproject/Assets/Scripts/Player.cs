...
public class Player : MonoBehaviour
{
    ...
    void Jump()
    {
        if (jDown && moveVec == Vector3.zero && !isJump && !isDodge && !isSwap && !isShop)  // 점프 한계설정  (움직임이 zero일때 그냥 점프, 액션 도중에 다른 액션 실행되지 않게 설계)
        {
    ...
    void Attack()
    {
        ...
        if (fDown && isFireReady && !isDodge && !isSwap && !isShop) //공격할때 같이 못누르는 값 설정
        {
    ...
    void Reload() //장전 함수 구현
    {
        ...
        if(rDown && !isJump && !isDodge && !isSwap && isFireReady && !isShop) //점프, 회피, 무기 변경때는 장전이 불가.
        {
    ...
    void Dodge()
    {
        if (jDown && moveVec != Vector3.zero && !isJump && !isDodge && !isSwap && !isShop)  // 점프 한계설정 (움직임이 zero가 아닐때 "회피(Dodge)")
        {
